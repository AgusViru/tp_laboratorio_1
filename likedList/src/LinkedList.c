...
{
	Node* nodeAux=NULL;
	    int lista= ll_len(this);
	    int i;
	    if(this!=NULL && nodeIndex>=0 && nodeIndex<lista)
	    {
...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	    ...
	    if(nodeIndex>=0 && nodeIndex<=lista)
	    {
	        ...
	        if(auxNode!=NULL)
	        {
	            ...
	            if(nodeIndex==0)
	            {
	                ...
	                {
	                    nodoUno= this->pFirstNode;
	                    nodoDos= nodoUno->pNextNode;
	                    while(nodeIndex>1)
	                    {
	                        nodoUno=nodoDos;
	                        nodoDos=nodoUno->pNextNode;
	                    ...
	                    nodoUno->pNextNode=auxNode;
	                    auxNode->pNextNode=nodoDos;
	                }
	                this->size++;
	                returnAux=0;
	        }
...
void* ll_get(LinkedList* this, int index)
{
		...
		Node* nodo;
	    int lista = ll_len(this);
	    if(this!=NULL && index>=0 && index<= lista)
	    {
...
{
		int returnAux = -1;
	    int auxiliarLongitud=ll_len(this);
	    Node* node;
	    Node* auxiliarNode;
	    if(this!=NULL && index>=0 && index<auxiliarLongitud)
	    {
	        if(index>0)
	        {
	            node=getNode(this,index);
	            auxiliarNode=getNode(this,index-1);
	            auxiliarNode->pNextNode=node->pNextNode;
	            free(node);
	            this->size--;
	            returnAux=0;
	        }
...
int ll_indexOf(LinkedList* this, void* pElement)
{
	    ...
	    if(this!=NULL)
	    {
	        ...
	        {
	            nodoUno = getNode(this,i);
	            if(nodoUno!=NULL && pElement==nodoUno->pElement)
	            {
	                returnAux=i;
	                break;
	            }
...
{
		int returnAux = -1;
	    int longitudUno=ll_len(this);
	    int longitudDos=ll_len(this2);
	    void* auxiliarElemento;
	    void* auxiliarElementosDos;
	    int i;
	    int contador=0;
	    if(this!=NULL && this2!=NULL)
	    {
	        for(i=0;i<longitudUno;i++)
	        {
	            ...
	            auxiliarElementosDos=ll_get(this2,i);
	            if(auxiliarElemento==auxiliarElementosDos)
	            {
	                contador++;
	            }
	        }
	        if(contador==longitudUno && longitudDos==longitudUno)
	        {
	            returnAux=1;
	        }
	        else
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
	    ...
	    Node* nodo;
	    int lista=ll_len(this);
	    if(this!=NULL && from>=0 && to>from && to<=lista)
	    {
	        ...
	        for(i=from,j=0;i<to;i++,j++)
	        {
	            ...
	            {
	                nodo=getNode(this,i);
	                addNode(cloneArray,j,nodo->pElement);
	            }
	        }
	    }
	        else
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
	int returnAux =-1;
	    ...
	    int flag;
	    int auxiliarFuncion;
	    if(this!=NULL && lista > 0 && pFunc!=NULL && (order == 0 || order == 1))
	    {
	        do
	        {
	            flag=0;
	            for(i=0;i<lista-1;i++)
	            {
	                ...
	                {
	                   auxiliarFuncion=pFunc(elementoUno,elementoDos);
	                   if((auxiliarFuncion==-1 && order==0) || (auxiliarFuncion==1 && order==1))
	                   {
	                        flag=1;
...
{
    LinkedList* otraLista=NULL;
    void* elemento;
    int i;
    int lista=ll_len(this);
    int auxiliarFuncion;
    if(this!=NULL && pFunc!=NULL && lista>0)
    {
        otraLista=ll_newLinkedList();
        for(i=0;i<lista;i++)
        {
            elemento=ll_get(this,i);
            if(elemento!=NULL)
            {
                auxiliarFuncion=pFunc(elemento);
                ll_add(otraLista,auxiliarFuncion);
            }
        }
    }
    return otraLista;
